...
| `--exact-size` | Force exact `--aspect` pixels (post-resize via `sips`). |
| `--json` | Machine-readable result. |
| `--timeout` | Seconds, default 900. |

Budget ~40–70s per image. Run generations for **different** assets in parallel
(separate background Bash calls) — runs are isolated by Codex thread ID.
